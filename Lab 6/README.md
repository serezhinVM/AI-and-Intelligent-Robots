...
- **6.2 — Improved map.** The environment map is now built from **many laser
  readings** (every MAP_STEP-th ray of the full 360° scan, ~90 per scan)
...
- **6.3 — Optimised code.** Magic numbers replaced by named constants
  (LEFT_LASER_INDEX, MAP_STEP, heading_*, odom_landmark*);
  	ransformMapPoint helper introduced; unused code removed.
...
- New 	ransformMapPoint(laserRange, laserTh) converts a single laser
