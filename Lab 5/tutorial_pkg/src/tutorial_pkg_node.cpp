...
public:
    ...
    void PID_moveto2ndGap(double moveSpeed, double robotHeading);
    void PID_pass2ndGap(double moveSpeed, double robotHeading);
    void PID_reachCharger(double moveSpeed, double robotHeading);
...
void Stopper::PID_wallFollowing(double forwardSpeed, double laserData){
    ...
    controller.kp = 0.5, controller.ki = 0.01;
    controller.kd = 0.01, controller.Max_output = 0.6;
...
void Stopper::PID_pass1stGap(double moveSpeed, double robotHeading){
    PID_para controller;
    double robotHeadingGap1 = 0;
    controller.kp = 2.0, controller.ki = 0.05;
    controller.kd = 0.01, controller.Max_output = 0.6;
    controller.ed_pre = 0, controller.ei_pre = 0;
...
void Stopper::PID_moveto2ndGap(double moveSpeed, double robotHeading){
    PID_para controller;
    double robotHeadingGap2 = 0;
    controller.kp = 2.0, controller.ki = 0.05;
    controller.kd = 0.01, controller.Max_output = 0.6;
    controller.ed_pre = 0, controller.ei_pre = 0;
    double PID_output = PID_control(controller, robotHeadingGap2, robotHeading);
    moveForwardRight(moveSpeed, PID_output);
}

void Stopper::PID_pass2ndGap(double moveSpeed, double robotHeading){
    PID_para controller;
    double robotHeading2ndGap = -1.57;
    controller.kp = 2.0, controller.ki = 0.05;
    controller.kd = 0.01, controller.Max_output = 0.6;
    controller.ed_pre = 0, controller.ei_pre = 0;
    double PID_output = PID_control(controller, robotHeading2ndGap, robotHeading);
    moveForwardRight(moveSpeed, PID_output);
}

void Stopper::PID_reachCharger(double moveSpeed, double robotHeading){
    PID_para controller;
    double robotHeadingCharger = 3.14;
    controller.kp = 2.0, controller.ki = 0.05;
    controller.kd = 0.01, controller.Max_output = 0.6;
    controller.ed_pre = 0, controller.ei_pre = 0;
    double PID_output = PID_control(controller, robotHeadingCharger, robotHeading);
    moveForwardRight(moveSpeed, PID_output);
}
    ...
    switch(stage){
        ...
        case 3:
            if (PositionX < odom_landmark3)
                PID_moveto2ndGap(FORWARD_SPEED_MIDDLE, robotHeadAngle);
            else stage = 4;
            break;
        case 4:
            if (PositionX < odom_landmark4)
                PID_pass2ndGap(FORWARD_SPEED_MIDDLE, robotHeadAngle);
            else stage = 5;
            break;
        case 5:
            if (PositionY > odom_landmark5)
                PID_reachCharger(FORWARD_SPEED_MIDDLE, robotHeadAngle);
            else stage = 6;
            break;
        case 6:
            moveStop();
